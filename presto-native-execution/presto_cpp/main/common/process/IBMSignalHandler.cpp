...
static void dumpSystemInfo(TrapFile& trapFile, HANDPARMS) {
  struct utsname name = {};
...
//   scp [in]
//     On Unix, scp is a Signal context structure.
static void dumpRegisters(TrapFile& trapFile, HANDPARMS) {
...
static std::string mCode(unsigned long instruction) {
...
//   scp [in]
//     On Unix, scp is a Signal context structure.
static void dumpStackTraceInternal(
    TrapFile& trapFile,
    HANDPARMS,
    uint32_t flags,
    uint32_t stackTop) {
  ...
  Uint8* const rip = reinterpret_cast<Uint8*>(r[REG_RIP]);
  void* syms[MAX_BACKTRACE_FRAMES_SUPPORTED];
  int cnt = 0;
  int i = 0;
  UintPtr* const rsp = reinterpret_cast<UintPtr*>(r[REG_RSP]);
...
static std::string signalToString(int signo) {
  switch (signo) {
    case SIGSEGV:
      return "SIGSEGV";
    case SIGILL:

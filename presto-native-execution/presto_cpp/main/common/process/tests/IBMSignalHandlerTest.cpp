...
class IBMSignalHandlerTest : public testing::Test {
 protected:
  ...
  std::string findPrestoServerExecutablePath() {
    ...
    const std::string prestoString = "/presto/";

    // Find the position of the "/presto/" substring.
    size_t pos = currentPath.find(prestoString);

    // If found, extract the path up to "/presto/" and set to prestoTop.
    if (pos != std::string::npos) {
      const std::string prestoTop =
          currentPath.substr(0, pos + prestoString.length() - 1);
      const std::string paths[] = {
          ...
          prestoTop + "/presto-native-execution/presto_cpp/main/presto_server"};
  ...
  void sendSignalToProcessAndTest(int signal) {
    ...
    pid_t pid = prestoServerProcess_->id();
